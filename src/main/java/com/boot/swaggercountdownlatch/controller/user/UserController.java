package com.boot.swaggercountdownlatch.controller.user;
import com.boot.swaggercountdownlatch.entity.User;
...
public class UserController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "username", value = "用户名", dataType = "string", defaultValue = "username111"),
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "用户编号", dataType ="DataType.LONG"),
    })
    ...
    public void putData(@PathVariable Long id, @RequestBody User user) {
        log.info("如果你不想写 @ApiImplicitParam 那么 swagger 也会使用默认的参数名作为描述信息 ");
    }

    @PutMapping("/{id}")
    @ApiOperation(value = "修改数据（DONE）")
    public void putData1(@PathVariable Long id, @RequestBody User user) {
        log.info("如果你不想写 @ApiImplicitParam 那么 swagger 也会使用默认的参数名作为描述信息 ");
    }

    @PutMapping("/{id}")
    @ApiOperation(value = "修改数据（DONE）")
    public void putData2(@PathVariable Long id, @RequestBody User user) {
        log.info("如果你不想写 @ApiImplicitParam 那么 swagger 也会使用默认的参数名作为描述信息 ");
    }
